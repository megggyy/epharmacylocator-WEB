...
const TaguigCityMap = () => {
  ...
  const handleSearch = (query) => {
    ...
    if (!query.trim()) {
      setFilteredPharmacies(showAllPharmacies ? pharmacies : []);
...
<div className="absolute top-4 right-4 z-50 flex flex-col items-start bg-white shadow-md p-2 w-96 rounded-lg">
  ...
  {pharmacySuggestions.length > 0 && (
    <ul className="absolute top-full mt-1 left-0 bg-white shadow-lg rounded-lg w-full max-h-60 overflow-y-auto z-50">
      ...
      {/* Map Container */}
      <MapContainer
        center={[region.latitude, region.longitude]} // Use user location for map center
        zoom={12}
       ...
       <MapResize />
        ...
        {filteredPharmacies.map((pharmacy) => (
          <Marker
            ...
            icon={pharmacyIcon} // Apply custom red icon
            eventHandlers={{
